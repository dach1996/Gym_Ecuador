CREATE TABLE [CORE].[ENTRENADORES] (
    ...
    [ENT_GUID]           UNIQUEIDENTIFIER CONSTRAINT [DEFAULT_ENTRENADORES_ENT_GUID] DEFAULT (newid()) NOT NULL,
    [ENT_FECHA_REGISTRO] DATETIME         NOT NULL,
    [PNA_ID]             INT              NOT NULL,
    [ENT_BIOGRAFIA]      VARCHAR (2000)   NULL,
    [ENT_ESTADO]         BIT              CONSTRAINT [DEFAULT_ENTRENADORES_ENT_ESTADO] DEFAULT ((1)) NOT NULL,
    [USR_ID_REGISTRADOR] INT              NULL,
    CONSTRAINT [PK_ENTRENADORES] PRIMARY KEY CLUSTERED ([ENT_ID] ASC),
    CONSTRAINT [FK_ENTRENADORES_PERSONA] FOREIGN KEY ([PNA_ID]) REFERENCES [AUTENTICACION].[PERSONA] ([PNA_ID]),
    CONSTRAINT [FK_ENTRENADORES_USUARIO_REGISTRADOR] FOREIGN KEY ([USR_ID_REGISTRADOR]) REFERENCES [AUTENTICACION].[USUARIO] ([USR_ID])
);
