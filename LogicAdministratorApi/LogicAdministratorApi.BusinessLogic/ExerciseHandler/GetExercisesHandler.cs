    ...
    IPluginFactory pluginFactory) : ExerciseBase<GetExercisesRequest, GetExercisesResponse>(logger, pluginFactory)
{
    ...
    public override async Task<GetExercisesResponse> Handle(GetExercisesRequest request, CancellationToken cancellationToken)
        => await ExecuteHandlerAsync(OperationAdministratorName.GetExercises, request, async () =>
            {
                // Construir el filtro
                ...
                if (!string.IsNullOrWhiteSpace(request.NameFilter))
                {
                    ...
                    whereFilter = exercise => exercise.Name.ToLower().Contains(nameFilter);
                }
                ...
                if (request.CatalogId.HasValue)
                {
                    ...
                    else
                    {
                        var nameFilter = request.NameFilter.ToLower();
                        whereFilter = exercise => exercise.Name.ToLower().Contains(nameFilter) && 
                                                 exercise.ExerciseTags.Any(et => et.CatalogId == request.CatalogId.Value);
                    }
                }
